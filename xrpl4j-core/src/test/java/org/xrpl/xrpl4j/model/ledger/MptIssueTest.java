...
class MptIssueTest {
  ...
  @Test
  void testEquals() {
    MptIssue issue1 = MptIssue.builder().mptIssuanceId(MPT_ID).build();
    MptIssue issue2 = MptIssue.builder().mptIssuanceId(MPT_ID).build();
  ...
  void testJsonSerialization() throws JsonProcessingException, JSONException {
    MptIssue issue = MptIssue.builder()
      .mptIssuanceId(MPT_ID)
      .build();

    String expectedJson = "{\"mpt_issuance_id\":\"" + MPT_ID.value() + "\"}";
    ...
    MptIssue deserialized = objectMapper.readValue(serialized, MptIssue.class);
