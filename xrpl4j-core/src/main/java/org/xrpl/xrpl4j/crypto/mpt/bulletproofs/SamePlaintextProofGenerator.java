...
public interface SamePlaintextProofGenerator {
  ...
  /**
   * Generates a proof that all ciphertexts in the participant list encrypt the same plaintext amount.
   *
   ...
   * @param participants The list of participants (each with ciphertext, publicKey, blindingFactor).
   *                     Must contain at least 2 participants.
   * @param context      The context hash for domain separation.
   *
   * @return A {@link SamePlaintextMultiProof}.
   *
   * @throws IllegalArgumentException if participants list has fewer than 2 elements.
   * @throws NullPointerException     if any parameter is null.
   */
  SamePlaintextMultiProof generateProof(
    UnsignedLong amount,
    List<SamePlaintextParticipant> participants,
