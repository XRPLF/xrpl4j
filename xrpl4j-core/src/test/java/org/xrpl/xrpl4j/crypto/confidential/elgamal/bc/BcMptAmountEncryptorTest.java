...
/**
 * Test for {@link BcElGamalEncryptor} comparing output with C implementation.
 */
@SuppressWarnings("checkstyle")
public class BcMptAmountEncryptorTest {

  private static JsonNode testVectors;
  private static final ObjectMapper objectMapper = new ObjectMapper();
  private final ElGamalEncryptor encryptor = new BcElGamalEncryptor();

  @BeforeAll
  static void loadTestVectors() throws Exception {
    InputStream is = BcMptAmountEncryptorTest.class.getResourceAsStream("/mpt/port/elgamal_encrypt_vectors.json");
  ...
  void encryptionMatchesCImplementation() {
    for (JsonNode vector : testVectors.get("vectors")) {
      ...
      EncryptedAmount javaCiphertext = encryptor.encrypt(amount, pubkeyQ, blindingFactor);
  ...
  void modifiedPubkeyProducesDifferentCiphertext() {
    ...
    for (JsonNode vector : testVectors.get("vectors")) {
      ...
      EncryptedAmount originalCiphertext = encryptor.encrypt(amount, pubkeyQ, blindingFactor);
      ...
      try {
        EncryptedAmount modifiedCiphertext = encryptor.encrypt(amount, modifiedPubkey, blindingFactor);
  ...
  void modifiedBlindingFactorProducesDifferentCiphertext() {
    ...
    for (JsonNode vector : testVectors.get("vectors")) {
      ...
      try {
        EncryptedAmount modifiedCiphertext = encryptor.encrypt(amount, pubkeyQ, modifiedBlindingFactor);
        assertThat(modifiedCiphertext.toBytes().hexValue())
