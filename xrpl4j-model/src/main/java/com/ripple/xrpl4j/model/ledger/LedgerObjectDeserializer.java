...
public class LedgerObjectDeserializer extends JsonDeserializer<LedgerObject> {
  ...
  Map<LedgerObject.LedgerEntryType, Class<? extends LedgerObject>> objectTypeMap = new ImmutableMap.Builder<LedgerObject.LedgerEntryType, Class<? extends LedgerObject>>()
  ...
  public LedgerObject deserialize(JsonParser jsonParser, DeserializationContext ctxt) throws IOException {
    ...
    return objectMapper.readValue(node.toString(), objectTypeMap.get(LedgerObject.LedgerEntryType.forValue(node.get("LedgerEntryType").asText())));
  }
}
