...
public class DefaultKeyPairService extends AbstractKeyPairService {
  ...
  /**
   * A map of the existing {@link KeyPairService} implementations, keyed by {@link KeyType}.
   */
  private static final Map<KeyType, Supplier<KeyPairService>> serviceMap =
    new ImmutableMap.Builder<KeyType, Supplier<KeyPairService>>()
      .put(KeyType.SECP256K1, Secp256k1KeyPairService::getInstance)
      .put(KeyType.ED25519, Ed25519KeyPairService::getInstance)
      .build();

  private static KeyPairService getKeyPairServiceByType(KeyType type) {
  ...
  /**
   * Generates a seed from the given entropy, encoded with {@link KeyType#ED25519}. Seeds can be generated using
  ...
  public String sign(UnsignedByteArray message, String privateKey) {
    ...
    KeyType privateKeyType = privateKey.startsWith("ED") ? KeyType.ED25519 : KeyType.SECP256K1;
  ...
  public boolean verify(UnsignedByteArray message, String signature, String publicKey) {
    // ED25519 keys are prefixed with "ED" to make them 33 bytes.
    KeyType publicKeyType = publicKey.startsWith("ED") ? KeyType.ED25519 : KeyType.SECP256K1;
