...
public class AccountTransactionsIT {
  ...
  void listTransactionsWithLedgerSpecifiers() throws JsonRpcClientErrorException {
    ...
    UnsignedLong validatedLedgerIndex = ledger.ledgerIndex()
      .orElseThrow(() -> new RuntimeException("ledgerIndex not present.")).unsignedLongValue();

    AccountTransactionsResult resultByLedgerIndex = getAccountTransactions(
      AccountTransactionsRequestParams.builder()
        .account(MAINNET_ADDRESS)
        .ledgerSpecifier(
          LedgerSpecifier.ledgerIndex(org.xrpl.xrpl4j.model.client.specifiers.LedgerIndex.of(validatedLedgerIndex))
        )
        .build()
    );
    ...
    assertThat(resultByShortcut.ledgerIndexMin().value()).isEqualTo(validatedLedgerIndex.longValue());
