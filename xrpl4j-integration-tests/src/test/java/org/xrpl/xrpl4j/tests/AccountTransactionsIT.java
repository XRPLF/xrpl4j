...
public class AccountTransactionsIT {
  ...
  // an arbitrary address on xrpl testnet that has a decent amount of transaction history
  public static final Address TESTNET_ADDRESS = Address.of("rURUD69hsgJYtrt1xJ4drhs45qRirg36CU");

  private final XrplClient testnetClient = new TestnetEnvironment().getXrplClient();

  @Test
  public void listTransactionsDefaultWithPagination() throws JsonRpcClientErrorException {
    AccountTransactionsResult results = testnetClient.accountTransactions(TESTNET_ADDRESS);
  ...
  public void listTransactionsPagination() throws JsonRpcClientErrorException {
    final int expectedTransactions = 421;
    // known ledger index range for this account that is known to have exactly 421 transactions
    LedgerIndexBound minLedger = LedgerIndexBound.of(26774141);
    LedgerIndexBound maxLedger = LedgerIndexBound.of(31614070);
    AccountTransactionsResult results = testnetClient.accountTransactions(
    ...
    while (results.marker().isPresent()) {
      results = testnetClient.accountTransactions(AccountTransactionsRequestParams
  ...
  public void listTransactionWithIndexRange() {
    ...
    LedgerIndexBound minLedger = LedgerIndexBound.of(26774141);
    LedgerIndexBound maxLedger = LedgerIndexBound.of(26936155);
    ...
    assertThat(results.transactions().get(0).resultTransaction().ledgerIndex())
      .isEqualTo(LedgerIndex.of(UnsignedInteger.valueOf(26935924)));
    ...
    assertThat(results.transactions().get(15).resultTransaction().ledgerIndex())
      .isEqualTo(LedgerIndex.of(UnsignedInteger.valueOf(26935923)));
  }
  ...
  void listTransactionsWithLedgerSpecifiers() throws JsonRpcClientErrorException {
    ...
    LedgerResult ledger = testnetClient.ledger(
  ...
  private AccountTransactionsResult getAccountTransactions(AccountTransactionsRequestParams params) {
    return given()
      ...
      .until(() -> {
        try {
          return testnetClient.accountTransactions(params);
