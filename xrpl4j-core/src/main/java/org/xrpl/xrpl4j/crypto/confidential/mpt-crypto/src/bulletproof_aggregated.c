...
static const unsigned char BP_FIXED_NONCES[10][32] = {
...
static int bp_nonce_run_index = -1;

void secp256k1_mpt_set_bp_nonce_run_index(int index) {
    bp_nonce_run_index = index;
}

/** Generates a secure 32-byte random scalar.
 * Returns 1 on success, 0 on failure.
 * When bp_nonce_run_index >= 0, returns fixed nonce for deterministic test vectors.
 */
static int generate_random_scalar(
        const secp256k1_context* ctx,
        unsigned char* scalar_bytes)
{
    if (bp_nonce_run_index >= 0) {
        memcpy(scalar_bytes, BP_FIXED_NONCES[bp_nonce_run_index], 32);
        return 1;
    }
    do {
