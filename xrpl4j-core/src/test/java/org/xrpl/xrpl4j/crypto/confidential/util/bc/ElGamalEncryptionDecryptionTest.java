...
public class ElGamalEncryptionDecryptionTest {
  ...
  void encryptThenDecryptWithDifferentBlindingFactors() {
    ...
    EncryptedAmount ct1 = encryptor.encrypt(amount, keyPair.publicKey(), bf1);
    EncryptedAmount ct2 = encryptor.encrypt(amount, keyPair.publicKey(), bf2);
  ...
  void decryptionFailsWhenC1BitIsFlipped() {
    ...
    EncryptedAmount ciphertext = encryptor.encrypt(amount, keyPair.publicKey(), blindingFactor);

    // Flip a bit in c1
    byte[] c1Bytes = ciphertext.c1().toByteArray();
    c1Bytes[c1Bytes.length - 1] ^= 0x01; // Flip last bit

    EncryptedAmount tamperedCiphertext = EncryptedAmount.of(
