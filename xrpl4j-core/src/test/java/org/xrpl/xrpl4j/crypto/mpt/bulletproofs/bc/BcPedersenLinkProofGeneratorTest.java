...
/**
 * Unit tests for {@link BcPedersenLinkProofGenerator}.
 *
...
class BcPedersenLinkProofGeneratorTest {
  ...
  private BcPedersenLinkProofGenerator generator;
  private PedersenCommitmentGenerator pedersenGen;
  private BcPedersenLinkProofVerifier verifier;

  @BeforeEach
  void setUp() {
    generator = new BcPedersenLinkProofGenerator();
    pedersenGen = new BcPedersenCommitmentGenerator();
  ...
  void testGenerateAndVerifyAmountCommitmentProof() {
    ...
    ECPoint rPk = Secp256k1Operations.multiply(Secp256k1Operations.toEcPoint(publicKey), rInt);
    ...
    boolean isInvalid = verifier.verify(
      LinkageProofType.AMOUNT_COMMITMENT,
      proof,
      ciphertext,
      publicKey,
  ...
  void testGenerateAndVerifyBalanceCommitmentProof() {
    ...
    KeyPair senderKeyPair = Seed.elGamalSecp256k1Seed().deriveKeyPair();
    ...
    ECPoint mG = Secp256k1Operations.multiplyG(mInt);
    ECPoint rPk = Secp256k1Operations.multiply(Secp256k1Operations.toEcPoint(senderPublicKey), rInt);
    ECPoint c2 = Secp256k1Operations.add(mG, rPk);
    ElGamalCiphertext ciphertext = new ElGamalCiphertext(c1, c2);
